...
//컴파일 : g++ -o server server.cpp -lpthread
//-lpthread
//실행 : ./server 9999
...
char *EXIT_STRING = "종료 되었습니다";	// 클라이언트의 종료요청 문자열
char *START_STRING = "연결되었습니다 \n";
// 클라이언트 환영 메시지
...
void *thread_function(void *arg) { //명령어를 처리할 스레드
	int i;
	printf("*help 명령어를 입력해주세요\n");
	while (1) {
		char bufmsg[MAXLINE + 1];
		fprintf(stderr, "\033[1;33m"); //글자색을 녹색으로 변경
		printf("server>"); //커서 출력
		fgets(bufmsg, MAXLINE, stdin); //명령어 입력
		if (!strcmp(bufmsg, "\n")) continue;   //엔터 무시
		else if (!strcmp(bufmsg, "*help\n"))    //명령어 처리
			printf("*help, *user, *chatnum, *iplist\n");
		else if (!strcmp(bufmsg, "*user\n"))//명령어 처리
			printf("참여자 수 = %d\n", num_user);
		else if (!strcmp(bufmsg, "*chatnum\n"))//명령어 처리
			printf("대화 갯수 = %d\n", num_chat);
	...
	while (1) {
		...
		if (FD_ISSET(listen_sock, &read_fds)) {
			...
			*/
			...
			printf("!채팅 참여. 현재 참여자 수 = %d\n", num_user);
			fprintf(stderr, "\033[33m");//녹색은 32
		...
		for (i = 0; i < num_user; i++) {
			if (FD_ISSET(clisock_list[i], &read_fds)) {
				...
				fprintf(stderr, "\033[33m");//흰색은 97m
				printf("%s", buf);			//메시지 출력
				fprintf(stderr, "\033[33m");//녹색은 32m
				fprintf(stderr, "server>"); //커서 출력
			}
		}
